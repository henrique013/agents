...
- O script exige `agents.bootstrap.skill: update-docs`; essa skill é reservada ao bootstrap e à autoatualização.
- Em `agents.root: true`, o script usa somente fontes locais deste repositório: `templates/AGENTS.tpl.md`, `remote.tpl_dir`, `local.tpl_dir` e `templates/skills/update-docs/`.
...
- O script sincroniza `.codex/skills/update-docs/`, se executa de novo com a cópia publicada atualizada e só então materializa as `conventions` no diretório configurado em `conventions.out_dir`, regenera `AGENTS.md` e publica os pacotes normais declarados em `outputs.skills`.
- `conventions.out_dir` define o diretório publicado.
- `local.tpl_dir` e `remote.tpl_dir` definem as raízes de resolução de cada origem.
- `entries[].from` é relativo ao `tpl_dir` da origem e pode conter subdiretórios.
...
- `outputs.skills` é opcional. Quando existir, `outputs.skills.out_dir` define o diretório publicado das skills.
...
- `outputs.skills.entries` não deve declarar `update-docs`, porque `.codex/skills/update-docs/` é reservado à skill de bootstrap.
...
- O lançador em `bin/agents-bootstrap.py` é o bootstrap inicial para instalar `.codex/skills/update-docs/` quando a skill ainda não existe localmente.
